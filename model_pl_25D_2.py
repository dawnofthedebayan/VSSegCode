...
class DoubleSegmentationLightningModel25D(pl.LightningModule):
    def __init__(self, model, loss_type="dice", optimizer="adam", lr=1e-3, weight_decay=1e-5, scheduler="cosine", enable_deep_supervision= False , input_type= "t1", num_classes=3 , args =None, **kwargs):
        ...
        if args is not None:
            ...
            self.group_name = args.input_type + '_' + args.unet_model + '_deep_sup_' + str(args.enable_deep_supervision) + '_num_classes_' + str(args.num_classes) + '_loss_' + args.loss_type + '_concat_' + str(args.concatanate_features) + '_encoder_attention_' + str(args.encoder_attention) + '_dataset_' + args.dataset
    ...
    def save_image(self, image, mask, predicted_mask,type="train", test_counter=0):
        ...
        predicted_mask = np.transpose(predicted_mask, (0,2,3,1))
        ...
        for i in range(image.shape[0]):
            ...
            nib.save(mask_, "/media/debayan/c7b64c90-ca4e-4192-8ed9-8fea1d005196/SchwanommaDS/UKE/code/inferrence/" + "/" + self.group_name + "/" + type +  "/gt_mask/" + self.run_name + "_" + str(i) + "_"  + str(test_counter) + "_mask.nii.gz")

            predicted_mask 
            # predicted mask
            predicted_mask_ = predicted_mask[i,:,:,:,:]
            # change value range suitable for nibabel
